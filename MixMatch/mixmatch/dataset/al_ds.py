...
class ChestnutSubset(ChestnutDataset):
    ...
    def to_tensor(self) -> torch.Tensor:
        return torch.from_numpy(self.data.transpose((0, 3, 1, 2))).contiguous()
...
class ChestnutSubsetKAug(ChestnutDataset):
    ...
    def concat(self, 
        ...
        """
        Adds new examples to itself
        """
        self.targets = np.concatenate((self.targets, new_labels))
        self.data = np.concatenate((self.data, new_samples))
        self.standardize()

    def remove(self,
               idxes: np.ndarray):
        self.targets = np.delete(self.targets, idxes, axis=0)
        self.data = np.delete(self.data, idxes, axis=0)
...
def get_datasets(
    ...
    num_workers: int = 0,
    seed: int = 42,
) -> tuple[Dataset, Dataset, Dataset, Dataset, list[str]]:
    """Get the dataloaders for the CIFAR10 dataset.
